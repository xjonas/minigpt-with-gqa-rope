...
class RotaryPositionalEmbeddings(nn.Module):
    """ 
    Implement RoPE introduced in the paper RoFormer: Enhanced Transformer with Rotary Position Embedding.
    ...
    def _build_cache(self, Y: torch.Tensor):
        ...
        #Y shape (batch, heads, seq_len, d)

        N = Y.shape[2]
        
        m = torch.arange(1, N + 1, device=Y.device, dtype=Y.dtype)
        theta = self.theta.to(Y.device, dtype=Y.dtype)

        C_inter = einsum(m, theta, 'n, d -> n d')
        C = torch.concat([C_inter, C_inter], dim = -1)
        self._cache = (torch.cos(C), torch.sin(C))

    def forward(self, Y: torch.Tensor):
        if self._cache == None or Y.shape[2] > self._cache[0].shape[0]:
            self._build_cache(Y) 
        ...
        # match the current sequence length of Y
        seq_len = Y.shape[2]
        cos = cos[:seq_len, :]
        sin = sin[:seq_len, :]

        Y_new = torch.cat([Y1, Y2], dim=-1) * cos + torch.cat([-Y2, Y1], dim=-1) * sin
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.rope:
            ...
            """
            self.query_rotary_pe = RotaryPositionalEmbeddings(d=self.n_embd // self.n_head) # Do NOT rename self.query_rotary_pe.
            self.key_rotary_pe = RotaryPositionalEmbeddings(d=self.n_embd // self.n_head) # Do NOT rename self.key_rotary_pe.
...
class GroupedQueryAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Key, Query, Value Projections: you must define these as nn.Linear().
    ...
    def forward(self, x):
        ...
        q = rearrange(q, 'b (kv g) t d -> b kv g t d', kv=self.n_kv_head)

        att = einsum(q, k, 'b kv g q d, b kv k d -> b kv g q k') / scale   

        att = att.masked_fill(self.bias[:,:,:t,:t] == 0, float('-inf'))
        att = F.softmax(att, dim=-1)
        att = self.attn_dropout(att)

        y = einsum(att, v, 'b kv g q t, b kv t d -> b kv g q d')
        end_memory = torch.cuda.memory_allocated()
        y = rearrange(y, 'b kv g t d -> b t (kv g d)')
